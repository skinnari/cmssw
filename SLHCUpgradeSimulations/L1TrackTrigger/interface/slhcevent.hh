...
public:
  ...
  void read(istream& in) {
	...
	for (unsigned int i=0;i<stubs_.size();i++) {
	  if (layer<10) {
	    ...
	    if (fabs(stubs_[i].r()-stub.r())<0.2&&
		stubs_[i].r()*fabs(stubs_[i].phi()-stub.phi())<0.02&&
		fabs(stubs_[i].z()-stub.z())<0.02) {
	      foundclose=true;
	    }
	  }
	}
  ...
  void allSector() {
    ...
    //cout << "allSector : "<<nstub<<" "<<stubs_.size()<<endl;
  }
