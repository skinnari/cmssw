            ...
            for x in memorymodules :
                if (basename==x[1]) :
                    ...
                    if (number>x[2]) :
                        x[2]=number
            if (not found) :
                memorymodules.append([4,basename,number])
...
for line in fi :
    ...
    if (splitline[0]=="TrackletProcessor:") :
        processingmodules.append([2,splitline[1]])
    if (splitline[0]=="MatchProcessor:") :
...
#           IL    AS   TPAR  FM    TF   CT
xmemories=[0.005,0.22, 0.43 ,0.67 ,0.82,0.96, 0.97]
dxmemories=[0.07,0.045,0.055,0.055,0.04,0.035,0.025]
...
#            VMR    TP   MP    FT   PD
xprocessing=[0.12,  0.31,0.55, 0.77, 0.90, 0.98]
dxprocessing=[0.035,0.06,0.035,0.035,0.035 ]
