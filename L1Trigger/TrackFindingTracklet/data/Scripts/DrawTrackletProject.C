...
public:
  ...
  void DrawConnection(const DrawBase& otherDrawBase) {
    TLine* l = new TLine(x2_, 0.5 * (y1_ + y2_), otherDrawBase.x1_, 0.5 * (otherDrawBase.y1_ + otherDrawBase.y2_));
    l->SetLineWidth(1);
    l->Draw();
  }
...
public:
  ...
  Box(double x1, double y1, double x2, double y2, TString name, unsigned int theMultiplicity = 1)
      : DrawBase(x1, y1, x2, y2, name) {
