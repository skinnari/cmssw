...
public:
  TETableInnerOverlap() { nbits_ = 10; }

  TETableInnerOverlap(int layer1, int disk2, int zbits, int rbits) {
    nbits_ = 10;
  ...
  void init(int layer1, int disk2, int zbits, int rbits) {
    layer1_ = layer1;
    disk2_ = disk2;
    zbits_ = zbits;
    rbits_ = rbits;
  ...
  int getLookupValue(int izbin, int irbin) {
    ...
    if (print)
      cout << "PRINT layer1 rmind2 rmaxd2 z2 r1 " << layer1_ << " " << rmind2 << " " << rmaxd2 << " " << z2 << " " << r1
           << endl;
    ...
    if (print)
      cout << "PRINT layer1 rmind2 rmaxd2 dr z2 r1 " << layer1_ << " " << rmind2 << " " << rmaxd2 << " "
           << " " << (rmaxdiskvm - rmindiskvm) / NBINS << z2 << " " << r1 << endl;

    if (print)
      cout << "PRINT rbminmin rbinmax " << rbinmin << " " << rbinmax << endl;
