...
public:
  TrackletCalculatorBase(string name, unsigned int iSector) : ProcessBase(name, iSector) {}
  ...
  void exacttracklet(double r1,
    ...
    for (int i = 0; i < 5; i++) {
      //int sign=1;
      //if (t<0) sign=-1;
      exactprojdisk(zmean[i], rinv, phi0, t, z0, phiprojdisk[i], rprojdisk[i], phiderdisk[i], rderdisk[i]);
    }
  }
  ...
  void exacttrackletdisk(double r1,
    ...
    if (disk_ == 1) {
      if (dumppars) {
        cout << "------------------------------------------------" << endl;
        cout << "DUMPPARS0:"
             << " dz= " << z2 - z1 << " rinv= " << rinv << " phi0= " << phi0 << " t= " << t << " z0= " << z0 << endl;
      }
    }
    ...
    for (int i = 0; i < 3; i++) {
      exactproj(rmean[i], rinv, phi0, t, z0, phiprojLayer[i], zprojLayer[i], phiderLayer[i], zderLayer[i]);
    }
  }
  ...
  bool inSector(int iphi0, int irinv, double phi0approx, double rinvapprox) {
    ...
    if (debug1)
      if (keepapprox && !keep)
        cout << getName()
             << " Tracklet kept with exact phicrit cut but not approximate, phicritapprox: " << phicritapprox << endl;
    if (usephicritapprox) {
      return keepapprox;
    } else {
      return keep;
  ...
  bool barrelSeeding(Stub* innerFPGAStub, L1TStub* innerStub, Stub* outerFPGAStub, L1TStub* outerStub) {
    if (debug1) {
      cout << "TrackletCalculator " << getName() << " " << layer_
           << " trying stub pair in layer (inner outer): " << innerFPGAStub->layer().value() << " "
           << outerFPGAStub->layer().value() << endl;
    }
    ...
    */
    ...
    if (layer_ > 3)
      iz1 <<= (nbitszL123 - nbitszL456);
    if (layer_ > 2)
      iz2 <<= (nbitszL123 - nbitszL456);
    ...
    if (writeTrackletPars) {
      ...
      out << "Trackpars " << layer_ << "   " << rinv << " " << rinvapprox << " " << ITC->rinv_final.get_fval() << "   "
    ...
    if (debug1) {
      cout << "TrackletCalculator " << getName() << " Found tracklet in layer = " << layer_ << " " << iSector_
           << " phi0 = " << phi0 << endl;
    }
    ...
    hists->fillTrackletParams(iSeed_,
                              iSector_,
                              rinvapprox,
  ...
  bool diskSeeding(Stub* innerFPGAStub, L1TStub* innerStub, Stub* outerFPGAStub, L1TStub* outerStub) {
    ...
    */
    ...
    if (writeTrackletParsDisk) {
      ...
      out << "Trackpars         " << disk_ << "   " << rinv << " " << rinvapprox << " " << ITC->rinv_final.get_fval()
          << "   " << phi0 << " " << phi0approx << " " << ITC->phi0_final.get_fval() << "   " << t << " " << tapprox
          << " " << ITC->t_final.get_fval() << "   " << z0 << " " << z0approx << " " << ITC->z0_final.get_fval()
          << endl;
    }
    ...
    if (debug1) {
      cout << "Found tracklet in disk = " << disk_ << " " << tracklet << " " << iSector_ << endl;
    }
  ...
  bool overlapSeeding(Stub* innerFPGAStub, L1TStub* innerStub, Stub* outerFPGAStub, L1TStub* outerStub) {
    ...
    if (writeTrackletParsOverlap) {
      ...
      out << "Trackpars " << disk_ << "   " << rinv << " " << irinv << " " << ITC->rinv_final.get_fval() << "   "
          << phi0 << " " << iphi0 << " " << ITC->phi0_final.get_fval() << "   " << t << " " << it << " "
    ...
    if (debug1) {
      cout << "Found tracklet in overlap = " << layer_ << " " << disk_ << " " << tracklet << " " << iSector_ << endl;
    }
