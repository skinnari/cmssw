...
public:
  MatchCalculator(string name, unsigned int iSector) : ProcessBase(name, iSector) {
  ...
  void addOutput(MemoryBase* memory, string output) {
    ...
    if (output == "matchout1" || output == "matchout2" || output == "matchout3" || output == "matchout4" ||
        output == "matchout5" || output == "matchout6" || output == "matchout7" || output == "matchout8" ||
        output == "matchout9") {
  ...
  void addInput(MemoryBase* memory, string input) {
    ...
    if (input == "matchin" || input == "match1in" || input == "match2in" || input == "match3in" ||
        input == "match4in" || input == "match5in" || input == "match6in" || input == "match7in" ||
        input == "match8in") {
      ...
      if (layer_ != 0) {
        ...
        if (debug1) {
          cout << getName() << " imatch = " << imatch << " ideltaphi cut " << ideltaphi << " "
               << phimatchcut_[seedindex] << " ideltaz*fact cut " << ideltaz * fact_ << " " << zmatchcut_[seedindex]
               << endl;
        }
        ...
        if (imatch) {
          ...
          for (unsigned int l = 0; l < fullmatches_.size(); l++) {
            if (debug1) {
              cout << getName() << " Trying to add match to: " << fullmatches_[l]->getName() << " " << tracklet->layer()
                   << " " << tracklet->disk() << " " << fullmatches_[l]->getName().substr(3, 4) << endl;
            }
            ...
            if (!extended_) {
              if ((layer == 1 && disk == 0 && fullmatches_[l]->getName().substr(3, 4) == "L1L2") ||
                ...
                if (writeSeeds) {
                  ...
                  fout << __FILE__ << ":" << __LINE__ << " " << name_ << "_" << iSector_ << " " << tracklet->getISeed()
                       << endl;
                  fout.close();
                }
    ...
    if (layer_ > 0) {
      ...
      for (unsigned int i = 1; i < tmp.size(); i++) {
        if (lastTCID > tmp[i].first.first->TCID()) {
          cout << "Wrong TCID ordering for projections in " << getName() << " last " << lastTCID << " "
               << tmp[i].first.first->TCID() << endl;
          error = true;
        } else {
      ...
      if (error) {
        for (unsigned int i = 1; i < tmp.size(); i++) {
          cout << "Wrong order for in " << getName() << " " << i << " " << tmp[i].first.first << " "
               << tmp[i].first.first->TCID() << endl;
        }
      }
    }
