...
public:
  ...
  Stub(const L1TStub& stub, double phiminsec, double phimaxsec) {
    ...
    if (layer < 999) {
      ...
      if (r < rmin || r > rmax)
        cout << "Error r, rmin, rmeas,  rmax :" << r << " " << rmin << " " << 0.5 * (rmin + rmax) << " " << rmax
             << endl;
      ...
      if (z < zmin || z > zmax)
        cout << "Error z, zmin, zmax :" << z << " " << zmin << " " << zmax << endl;
      ...
      if (r < rmin || r > rmax)
        cout << "Error disk r, rmin, rmax :" << r << " " << rmin << " " << rmax << endl;
  ...
  std::string str() const {
    ...
    if (layer_.value() != -1) {
      ...
      if (isPSmodule())
        ...
        oss << "000" << r_.str() << "|" << z_.str() << "|" << phi_.str() << "|" << alphanew_.str() << "|"
            << bend_.str();
    }
  ...
  double zapprox() const {
    ...
    if (disk_.value() < 0)
      sign = -1;
    if (sign < 0) {
      return (z_.value() + 1) * kz +
             sign *
                 zmean[abs(disk_.value()) - 1];  //Not sure why this is needed to get agreement with integer calculations
    } else {
