...
public:
  ...
  void trackFitNew(FPGATracklet* tracklet){
    ...
    //FPGAWord layermasktmp;
    //layermasktmp.set(layermask,6);
    //cout << "layermask diskmask alphaindex: "
    //	 <<layermasktmp.str()<<" "<<diskmask<<" "<<alphaindex<<endl;
    ...
    */
    ...
    for(unsigned int j=0;j<2*n;j++) {
      ...
      //cout << "j = "<<j<<" drinv, drinvexact = "<<MinvDt[0][j]*delta[0]<<" "
      //     <<MinvDt[0][j]*deltaexact[j]<<" "<<delta[j]<<" "
      //     <<deltaexact[j]<<endl;
    ...
    //cout << "rinvseed rinvseedexact irinvseed drinv drinvexact idrinv : "
    //	 <<rinvseed<<" "<<rinvseedexact<<" "<<irinvseed*krinvpars<<" " 
    //	 <<drinv<<" "<<drinvexact<<" "<<(idrinv>>fitrinvbitshift)*krinvpars<<endl;

    
    //cout << "tseed tseedexact itseed dt dtexact idt : "
    //	 <<tseed<<" "<<tseedexact<<" "<<itseed*ktpars<<" " 
    //	 <<dt<<" "<<dtexact<<" "<<(idt>>fittbitshift)*ktpars<<endl;
...
////////////// END NEW CHISQ /////////////////////
  ...
  void execute(std::vector<FPGATrack*>& tracks) {
    ...
    if (writeFitTrack) {
      ...
      static ofstream out1("fittrack1.txt");
	...
	if (getName()=="FT_L1L2") nmatchdisk=tracklet->nMatchesDisk(true);
	if (nmatch+nmatchdisk!=it->second) out1 << "xxx ";
	out1 << getName()<<" "<<nmatch<<" "<<nmatchdisk<<" "
	     <<it->second<<" "<<tracklet<<endl;
	int count=it->second;
	if (count>4) count=4;
	matches[count-1]++;
      }
