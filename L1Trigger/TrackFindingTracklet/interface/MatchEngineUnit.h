...
public:
 ...
 MatchEngineUnit(bool barrel,
		 vector<bool> table,
		 vector<bool> tablePS,
		 vector<bool> table2S):candmatches_(5){
    idle_=true;
    barrel_=barrel;
    table_=table;
    tablePS_=tablePS;
  ...
  bool empty() const {
    return candmatches_.empty();
  }

  std::pair<Tracklet*,std::pair<Stub*,L1TStub*> > read() {
    return candmatches_.read();
  }

  
  std::pair<Tracklet*,std::pair<Stub*,L1TStub*> > peek() const {
    return candmatches_.peek();
  }
