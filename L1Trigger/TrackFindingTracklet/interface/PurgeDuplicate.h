...
public:
  PurgeDuplicate(string name, unsigned int iSector) : ProcessBase(name, iSector) {}
  ...
  void addInput(MemoryBase* memory, string input) {
    ...
    if (input == "trackin" || input == "trackin1" || input == "trackin2" || input == "trackin3" ||
        input == "trackin4" || input == "trackin5" || input == "trackin6" || input == "trackin7" ||
        input == "trackin8" || input == "trackin9" || input == "trackin10" || input == "trackin11") {
    ...
    if (RemovalType == "grid") {
      ...
      for (unsigned int itrk = 0; itrk < numTrk; itrk++) {
        if (inputtracks_[itrk]->duplicate())
          cout << "WARNING: Track already tagged as duplicate!!" << endl;
        ...
        if (phiTest < -2 * M_PI / 27)
          cout << "track phi too small!" << endl;
        if (phiTest > 2 * 2 * M_PI / 27)
          cout << "track phi too big!" << endl;
      }
    }  // end grid removal
          ...
          if (RemovalType == "ichi") {
            if ((nStubP - nShare[jtrk] < minIndStubs) || (nStubS[jtrk] - nShare[jtrk] < minIndStubs)) {
              if ((int)inputtracks_[itrk]->ichisq() / (2 * inputtracks_[itrk]->stubID().size() - 4) >
                ...
                cout << "Error: Didn't tag either track in duplicate pair." << endl;
            }
          }  // end ichi removal
