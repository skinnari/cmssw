...
public:
  ...
  std::string trackletparstr() {
    std::ostringstream oss;
    if (writeoutReal) {
      oss << fpgapars_.rinv().value() * krinvpars << " " << fpgapars_.phi0().value() * kphi0pars << " "
          << fpgapars_.d0().value() * kd0pars << " " << fpgapars_.z0().value() * kz << " "
    ...
    if (!writeoutReal) {
      ...
      oss << outerFPGAStub_->stubindex().str() << "|" << fpgapars_.rinv().str() << "|" << fpgapars_.phi0().str() << "|"
  ...
  void addMatchDisk(int disk,
    ...
    diskresid_[abs(disk) - 1].init(disk,
                                   ideltaphi,
                                   ideltar,
                                   stubid,
                                   dphi,
  ...
  FPGAWord ichiSqfit() const { return FPGAWord(ichisqrphifit_.value() + ichisqrzfit_.value(), ichisqrphifit_.nbits()); }
  ...
  std::string trackfitstr() {
    ...
    if (writeoutReal) {
      oss << (fpgafitpars_.rinv().value()) * krinvpars << " " << (fpgafitpars_.phi0().value()) * kphi0pars << " "
          << (fpgafitpars_.d0().value()) * kd0pars << " " << (fpgafitpars_.t().value()) * ktpars << " "
    ...
    if (!writeoutReal) {
      oss << fpgafitpars_.rinv().str() << "|" << fpgafitpars_.phi0().str() << "|" << fpgafitpars_.d0().str()
          << "|"
          //<< "xxxxxxxxxxx|"
          << fpgafitpars_.t().str() << "|" << fpgafitpars_.z0().str()
          << "|"
          //<< ichisqfit_.str()<< "|"
          << innerFPGAStub_->phiregionaddressstr() << "|";
      if (middleFPGAStub_) {
  ...
  int overlap() const { return innerStub_->layer() + 21; }
  ...
  int seed() const {
    ...
    if (disk_) {
      if (innerStub_->disk() < 0)
        return innerStub_->disk() - 10;
      else
        return innerStub_->disk() + 10;
    }
    if (overlap_) {
      if (innerStub_->disk() < 0)
        return -21 - outerStub_->layer();
      else
        return 21 + outerStub_->layer();
    }
    return 0;
