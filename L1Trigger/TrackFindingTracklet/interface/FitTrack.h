...
public:
  FitTrack(string name, unsigned int iSector) : ProcessBase(name, iSector) { trackfit_ = 0; }
  ...
  void addInput(MemoryBase* memory, string input) {
    ...
    if (input == "tparin" || input == "tpar1in" || input == "tpar2in" || input == "tpar3in" || input == "tpar4in" ||
        ...
        input == "tpar10in" || input == "tpar11in" || input == "tpar12in" || input == "tpar13in" ||
        input == "tpar14in" || input == "tpar15in") {
    ...
    if (input == "fullmatch1in1" || input == "fullmatch1in2" || input == "fullmatch1in3" || input == "fullmatch1in4" ||
        ...
        input == "fullmatch1in9" || input == "fullmatch1in10" || input == "fullmatch1in11" ||
        input == "fullmatch1in12" || input == "fullmatch1in13" || input == "fullmatch1in14" ||
        input == "fullmatch1in15" || input == "fullmatch1in16") {
    ...
    if (input == "fullmatch2in1" || input == "fullmatch2in2" || input == "fullmatch2in3" || input == "fullmatch2in4" ||
        ...
        input == "fullmatch2in9" || input == "fullmatch2in10" || input == "fullmatch2in11" ||
        input == "fullmatch2in12" || input == "fullmatch2in13" || input == "fullmatch2in14" ||
        input == "fullmatch2in15" || input == "fullmatch2in16") {
      FullMatchMemory* tmp = dynamic_cast<FullMatchMemory*>(memory);
      assert(tmp != 0);
    ...
    if (input == "fullmatch3in1" || input == "fullmatch3in2" || input == "fullmatch3in3" || input == "fullmatch3in4" ||
        ...
        input == "fullmatch3in9" || input == "fullmatch3in10" || input == "fullmatch3in11" ||
        input == "fullmatch3in12" || input == "fullmatch3in13" || input == "fullmatch3in14" ||
        input == "fullmatch3in15" || input == "fullmatch3in16" || input == "fullmatch3in17") {
      FullMatchMemory* tmp = dynamic_cast<FullMatchMemory*>(memory);
      assert(tmp != 0);
    ...
    if (input == "fullmatch4in1" || input == "fullmatch4in2" || input == "fullmatch4in3" || input == "fullmatch4in4" ||
        ...
        input == "fullmatch4in9" || input == "fullmatch4in10" || input == "fullmatch4in11" ||
        input == "fullmatch4in12" || input == "fullmatch4in13" || input == "fullmatch4in14" ||
        input == "fullmatch4in15" || input == "fullmatch4in16" || input == "fullmatch4in17") {
      FullMatchMemory* tmp = dynamic_cast<FullMatchMemory*>(memory);
      assert(tmp != 0);
...
#ifdef USEHYBRID
    void trackFitNew(Tracklet * tracklet,
      ...
      if (derivatives == 0) {
        if (warnNoDer) {
          ...
          cout << "No derivative for layermask, diskmask : " << layermask << " " << tmpl.str() << " " << diskmask << " "
               << tmpd.str() << " eta = " << asinh(t) << endl;
        }
        return;
      }
      ...
      if (exactderivatives) {
        TrackDerTable::calculateDerivatives(
            nlayers, r, ndisks, z, alpha, t, rinv, D, iD, MinvDt, iMinvDt, sigma, kfactor);
        ttabi = t;
        ttab = t;
      } else {
        if (exactderivativesforfloating) {
          if (useMSFit) {
            TrackDerTable::calculateDerivativesMS(
                nlayers, r, ndisks, z, alpha, t, rinv, D, iD, MinvDt, iMinvDt, sigma, kfactor, ptbin);
          } else {
            TrackDerTable::calculateDerivatives(
                nlayers, r, ndisks, z, alpha, t, rinv, D, iD, MinvDt, iMinvDt, sigma, kfactor);
          }
...
*/
       ...
       */
        ...
        if (0 && j % 2 == 0) {
          cout << "DUMPFITLINNEW1"
               << " " << j << " " << rinvseed << " + " << MinvDt[0][j] * delta[j] << " " << MinvDt[0][j] << " "
               << delta[j] * rstub[j / 2] * 10000 << endl;

          cout << "DUMPFITLINNEW2"
               << " " << j << " " << tracklet->fpgarinv().value() * krinvpars << " + "
               << ((iMinvDt[0][j] * idelta[j])) * krinvpars / 1024.0 << " "
               << iMinvDt[0][j] * krinvpars / kphiprojdisk / 1024.0 << " "
               << idelta[j] * kphiproj123 * rstub[j / 2] * 10000 << " " << idelta[j] << endl;
        }
      }
      ...
      for (unsigned int i = 0; i < n; i++) {  // loop over stubs
        ...
        phifactor =
            rstub[k / 2] * delta[k] / sigma[k] + D[0][k] * drinv + D[1][k] * dphi0 + D[2][k] * dt + D[3][k] * dz0;

        iphifactor = kfactor[k] * rstub[k / 2] * idelta[k] * (1 << chisqphifactbits) / sigma[k] - iD[0][k] * idrinv -
        ...
        if (NewChisqDebug) {
          ...
          cout << "sum = " << phifactor - delta[k] / sigma[k] << "    drinvterm = " << D[0][k] * drinv
               << "  dphi0term = " << D[1][k] * dphi0 << "  dtterm = " << D[2][k] * dt
               << "  dz0term = " << D[3][k] * dz0 << endl;
        ...
        irzfactor = kfactor[k] * idelta[k] * (1 << chisqzfactbits) / sigma[k] - iD[0][k] * idrinv - iD[1][k] * idphi0 -
                    iD[2][k] * idt - iD[3][k] * idz0;
        ...
        if (NewChisqDebug) {
          cout << "delta[k]/sigma = " << delta[k] / sigma[k] << "  delta[k] = " << delta[k] << endl;
          cout << "sum = " << rzfactor - delta[k] / sigma[k] << "    drinvterm = " << D[0][k] * drinv
               << "  dphi0term = " << D[1][k] * dphi0 << "  dtterm = " << D[2][k] * dt
               << "  dz0term = " << D[3][k] * dz0 << endl;
    ...
    void trackFitFake(
        Tracklet * tracklet, std::vector<std::pair<Stub*, L1TStub*>>&, std::vector<std::pair<int, int>>&) {
    ...
    std::vector<Tracklet*> orderedMatches(vector<FullMatchMemory*> & fullmatch) {
      ...
      for (unsigned int i = 0; i < fullmatch.size(); i++) {
        ...
        if (debug1 && fullmatch[i]->nMatches() != 0)
          cout << "orderedMatches: " << fullmatch[i]->getName() << " " << fullmatch[i]->nMatches() << "\n";
      ...
      for (unsigned int i = 0; i < tmp.size(); i++) {
        if (i > 0) {
          ...
          if (tmp[i - 1]->TCID() > tmp[i]->TCID()) {
            cout << "Wrong TCID ordering in " << getName() << " : " << tmp[i - 1]->TCID() << " " << tmp[i]->TCID()
                 << endl;
            //assert(0);
          }
        }
    ...
    void execute() {
      ...
      do {
        ...
        if (debug1)
          cout << getName() << " : nMatches = " << nMatches << " " << asinh(bestTracklet->t()) << "\n";
